  ...
  return (
    <header className="border-outline-variant bg-surface/90 fixed top-0 left-0 z-100 w-full border-b backdrop-blur-xl">
      ...
      <nav aria-label="Navigație principală" className="hidden items-center gap-6 lg:flex">
        {navLinks.map((link) => (
          <a
            key={link.href}
            href={link.href}
            className="font-technical-data text-technical-data text-on-surface-variant hover:text-primary focus-visible:text-primary border-b border-transparent tracking-widest uppercase transition-colors"
          >
            {link.label}
          </a>
        ))}
      ...
      <div className="flex items-center gap-3">
        ...
        <a
          href="#contact"
          className="bg-primary-container text-on-primary-container font-technical-data text-technical-data hover:bg-primary hover:text-on-primary hidden px-6 py-2 tracking-widest uppercase transition-all sm:inline-block"
        >
          {"Cereți ofertă"}
        </a>
        <button
        ...
        >
          ...
          {navLinks.map((link) => (
            <a
              ...
              className="font-technical-data text-technical-data text-on-surface-variant hover:text-primary tracking-widest uppercase transition-colors"
            >
              {link.label}
            </a>
          ))}
          <a
            href="#contact"
            onClick={() => setOpen(false)}
            className="bg-primary-container text-on-primary-container font-technical-data text-technical-data px-6 py-3 text-center tracking-widest uppercase"
          >
            {"Cereți ofertă"}
          </a>
          </div>
