...
//..................................................... //
/**
 * PATCH ALMEK (2 modificari fata de sursa Skiper):
 *  1. Componenta era necontrolabila — starea era interna si nu exista niciun
 *     onClick expus, deci butonul se anima dar nu schimba tema. Documentatia
 *     Skiper trimite la hook-ul din skiper26; noi avem deja logica proprie
 *     (data-theme + preferinta sistemului + script anti-flash), asa ca am
 *     facut-o controlabila. Fara `isDark`/`onToggle` se comporta ca originalul.
 *  2. `bg-black`/`bg-white` hardcodate au fost inlocuite cu tokenii nostri,
 *     care se inverseaza singuri odata cu tema.
 * Celelalte patru variante sunt neatinse.
 */
export const ThemeToggleButton4 = ({
  ...
  const isDark = isControlled ? controlledIsDark : uncontrolledIsDark;
  return (
    <button
      type="button"
      aria-label={ariaLabel}
      ...
      className={cn(
        "rounded-full transition-all duration-300 active:scale-95",
        "bg-surface-container-high text-on-surface hover:text-primary border-outline-variant hover:border-primary border",
        className,
      )}
      onClick={() => {
        if (isControlled) onToggle?.();
        else setUncontrolledIsDark(!uncontrolledIsDark);
      }}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
